...
export default function AllProjectsGrid({ projects }: Props) {
  ...
  const startSlideshow = useCallback(
    (projectId: string, frameCount: number) => {
      ...
      hoverTimeoutRef.current = setTimeout(() => {
        slideIntervalRef.current = setInterval(() => {
          ...
          // Prepare previous and next indexes for the slide.
          ...
          // Start the slide on the next animation frame so initial
          // positions (0% and +100%) are applied before we animate.
      ...
      {projects.map((project) => {
        ...
        const allThumbs =
          project.pageThumbs && project.pageThumbs.length > 0
            ...
            : project.preview
              ? [project.preview]
              : [];
        const isHovering = hoveredProjectId === project.id;
        const activeIndex =
        ...
        return (
          <Link
            ...
            onMouseEnter={() => {
              if (allThumbs.length > 1 && hoveredProjectId !== project.id) {
                startSlideshow(project.id, allThumbs.length);
              }
            }}
            onMouseLeave={() => {
              stopSlideshow();
                      ...
                      {previousPreview && (
                        <Image
                          ...
                          onTransitionEnd={(event) => {
                            ...
                            // Only clear if this slide is still the active one.
                            setIsSliding(false);
                            setPrevPageIndex((current) =>
