        ...
        return (
          <RevealOnScroll key={tier.name} as="div" delayMs={index * 80} className="h-full">
            <div className="group relative h-full w-full overflow-visible">
              <div
                className={`relative z-10 flex h-full flex-col overflow-hidden rounded-2xl border-2 bg-white shadow-[0_6px_18px_rgba(15,23,42,0.06)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_20px_rgba(15,23,42,0.08)] ${
                  "border-slate-300"
                }`}
              >
                <div className="relative z-10 flex h-full flex-col px-6 pt-5 pb-6">
                    ...
                    <div className="pt-0.5">
                      ...
                      <div className="mt-5">
                        <div className="flex items-center gap-2">
                          <p
                            key={displayedPrice}
                            className="price-swap whitespace-nowrap text-[2.6rem] font-normal leading-tight tracking-tight text-slate-900 md:text-[3rem]"
                          >
                            {priceAmount}
                          </p>
                          {priceSuffix ? (
                            <span key={priceSuffix} className="price-swap text-base font-medium text-slate-500">
                              per {priceSuffix}
                            </span>
                          ) : null}
                        </div>
                        ...
                        >
                          <p className="text-sm font-medium text-slate-500">Billed annually</p>
                          {billingPeriod === "annual" ? (
                            <span className="rounded-full bg-purple-500 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-white">
                              {tier.annualSavings}
                            </span>
                          ) : null}
                        </div>
                  ...
                  <button
                    ...
                    className={`${
                      ...
                      isCurrentPlan
                        ...
                        : "bg-slate-900 text-white hover:-translate-y-px hover:bg-slate-800"
                  ...
                  {secondaryActionLabel && onSecondaryAction ? (
                    <div className="mt-3 flex flex-col items-center gap-2 text-xs font-medium text-slate-900/80">
                      <button
                        type="button"
                        ...
                        <li key={feature} className="flex items-center gap-2">
                          <span
                            className={`inline-flex h-5 w-5 flex-none items-center justify-center rounded-full ${
                              tier.name === "Essential Plus" ? "bg-sky-500" : "bg-indigo-500"
                            }`}
                          >
                            <Check className="h-3 w-3 text-white" strokeWidth={2.5} aria-hidden="true" />
                          </span>
                          <span className="font-medium leading-5 text-slate-900">{feature}</span>
                        </li>
                      );
                    })}
                  </ul>
